...
def mock_entity_registry():
    ...
    registry.async_get = Mock(side_effect=lambda entity_id: registry.entities.get(entity_id))
...
async def test_migration_requires_spook(mock_hass, mock_service_call):
    ...
    with patch("custom_components.powerwall_dashboard_energy_import._LOGGER") as mock_logger:
...
async def test_discover_teslemetry_entities(mock_hass, mock_entity_registry):
    ...
    mock_entity_registry.entities["sensor.tesla_site_battery_energy_in"] = battery_entity
...
@pytest.mark.asyncio
async def test_full_migration_dry_run(mock_hass, mock_service_call, mock_entity_registry):
    ...
    with patch("custom_components.powerwall_dashboard_energy_import.async_get_entity_registry", return_value=mock_entity_registry), \
         patch("custom_components.powerwall_dashboard_energy_import._discover_teslemetry_entities") as mock_discover, \
         patch("custom_components.powerwall_dashboard_energy_import._extract_teslemetry_statistics") as mock_extract, \
         patch("custom_components.powerwall_dashboard_energy_import._LOGGER") as mock_logger:
        ...
        mock_logger.info.assert_any_call("DRY RUN: Would import %d statistics for %s", 1, "sensor.powerwall_dashboard_home_usage_daily")
        mock_logger.info.assert_any_call("DRY RUN COMPLETE: Would migrate %d total statistics entries", 1)
...
def test_entity_pattern_matching():
    ...
    for teslemetry_entity, expected_pattern in test_cases:
        ...
        assert found_pattern == expected_pattern, f"Failed to match {teslemetry_entity} to {expected_pattern}"
...
def test_service_data_format():
    ...
    expected_fields = ["auto_discover", "entity_mapping", "start_date", "end_date", "dry_run", "overwrite_existing", "merge_strategy"]
