...
async def test_backfill_end_date_iso_format():
    ...
    call = Mock(spec=ServiceCall)
    call.hass = mock_hass
    call.data = {"start": "2024-01-01", "end": "2024-01-02T10:00:00Z"}
    
    # Should return early due to no entries, but hit date parsing
    await async_handle_backfill(call)
    assert True
...
def test_get_statistics_service_data_edge_cases():
    ...
    from custom_components.powerwall_dashboard_energy_import import _get_statistics_service_data
    
    # Test basic case
    result = _get_statistics_service_data("2024-01-01T00:00:00Z", "2024-01-02T00:00:00Z", "sensor.test")
    assert "statistic_ids" in result
    assert "period" in result
    
    # Test with None end time
    result = _get_statistics_service_data("2024-01-01T00:00:00Z", None, "sensor.test")
    assert result["end_time"] is None
    
    # Test with empty entity_id
    result = _get_statistics_service_data("2024-01-01T00:00:00Z", "2024-01-02T00:00:00Z", "")
    assert result["statistic_ids"] == [""]
...
async def test_date_parsing_edge_cases():
    ...
    from custom_components.powerwall_dashboard_energy_import import async_handle_backfill
