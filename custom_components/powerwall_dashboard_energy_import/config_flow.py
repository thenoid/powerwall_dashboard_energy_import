        ...
        if user_input is not None:
            if not await self._async_test_connection(self.hass, user_input):
                ...
                return self.async_create_entry(
                    title=user_input.get(CONF_PW_NAME, DEFAULT_PW_NAME), data=user_input
                )
...
class OptionsFlowHandler(config_entries.OptionsFlow):
    ...
    async def async_step_main(self, user_input=None):
        ...
        schema = vol.Schema(
            {
                vol.Required(
                    OPT_DAY_MODE, default=current.get(OPT_DAY_MODE, DEFAULT_DAY_MODE)
                ): vol.In(["local_midnight", "rolling_24h", "influx_daily_cq"]),
                vol.Required(
                    OPT_SERIES_SOURCE,
                    default=current.get(OPT_SERIES_SOURCE, DEFAULT_SERIES_SOURCE),
                ): vol.In(["autogen.http", "raw.http"]),
                vol.Required(
                    OPT_CQ_TZ, default=current.get(OPT_CQ_TZ, DEFAULT_CQ_TZ)
                ): str,
            }
        )
