    ...
    async def async_step_user(self, user_input=None) -> FlowResult:
        ...
        if user_input is not None:
            if not await self._async_test_connection(self.hass, user_input):
                ...
                # Use user-provided name as the entry title
...
class OptionsFlowHandler(config_entries.OptionsFlow):
    ...
    async def async_step_main(self, user_input=None):
        ...
        schema = vol.Schema({
            vol.Required(CONF_PW_NAME, default=current.get(CONF_PW_NAME, self.entry.data.get(CONF_PW_NAME, DEFAULT_PW_NAME))): str,
